...
14я ячейка - номер блока, который нужно добавить в стек после 15го.
15я ячейка - номер блока, который нужно добваить в стек.
16я ячейка используется для операций со стеком.
Ячейки с 17ой по S+17 - стек
...
Ячейка S+18 всегда должна быть равна нулю. Она используется для доступа к последней ячейке стека.

Ячейки с S+19 (с 29ой) можно использовать для всего
...
+[>
    ...
    [
        ...
        @e>>>>>>>>>>>>>@1b !0<<<<<<<<<<<<<  Ассерты для проверки что не вышли за границы стека (ячейка S плюс 18 все еще равна 0)
        [->>[>]<+[<]<]@e
    ]
...
=   Блок 1      Инициализация чисел в 28ой и 29ой ячейке (ИСПРАВИТЬ оно затирает стек)
    -<[-]+<[-]+>>[<-<->]<[<-]>[->
        >>>>>>>>>>>>>>>>>>>>>>>++++++++>+++++<<<<<<<<<<<<<<<<<<<<<<<<
    <]>
